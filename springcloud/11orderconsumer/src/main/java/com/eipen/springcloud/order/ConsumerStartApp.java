...
@EnableFeignClients
@RibbonClient(name = "10ITEMPROVIDER-EUREKA",configuration = MyConfiguration.class)//第二种方式 设置负载均衡的策略， 使用注解的方式， 但是需要创建一个config 类来声明一个对象（负载均衡规则的对象），然后需要注意的是这个类必须要在主运行程序的外层包下，也就是说不让自动扫描的程序扫这个类，否则会导致所有的服务都成了这个负载均衡的模式
