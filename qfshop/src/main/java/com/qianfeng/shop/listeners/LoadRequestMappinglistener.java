       ...
       // System.err.println("这是最终获取到的所有的类:"+allClassNames);
        ...
        allClassNames.forEach(className->{
            try {
                ...
                String pre = "";
                RequestMappping annotation = (RequestMappping) clazz.getAnnotation(RequestMappping.class);//从类上面找执行名字的注解
                if (annotation != null) {
                    //说明类上面有这个注解
                ...
                for (Method method : methods) {
                    //获取方法上面指定类型的注解
                    RequestMappping requestMappping = method.getAnnotation(RequestMappping.class);
                    if (requestMappping != null) {
                        //有注解
                        String path = requestMappping.value();//获取到方法上面路径
