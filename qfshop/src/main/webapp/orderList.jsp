			...
			<c:forEach items="${ordersList}" var="order" varStatus="i">
				<tr>
					...
					<th>
						...
						<c:if test="${order.ostate eq 0 }">
							<button type="button" class="btn btn-warning btn-sm" onclick="zhifu('${order.oid}')">支付</button>
							<button type="button" class="btn btn-warning btn-sm" onclick="quxiao('${order.oid}')">取消</button>
						</c:if>
						<c:if test="${order.ostate eq 2 }">
