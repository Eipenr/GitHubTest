	    	...
	    	<c:forEach items="${pageBean.list}" var="g" varStatus="i">
		    	<div class="col-sm-3">
				    <div class="thumbnail">
				      ...
				      <div class="caption">
				        <h4>商品名称<a href="${pageContext.request.contextPath}/product?method=detail&id=${g.id}">${g.name}</a></h4>
<%--				        <p>热销指数--%>
		...
		<nav aria-label="Page navigation">
			<ul class="pagination">
				<li class="${pageBean.currentPage==1?'disabled':''}">
					<a  href="product?method=show&id=${param.id}&currentPage=${pageBean.currentPage-1}" aria-label="Previous">
				...
				<c:forEach begin="1" end="${pageBean.totalPage}" step="1" var="index">
					<c:if test="${pageBean.currentPage==index}">
						<li class="active"><a href="product?method=show&id=${param.id}&currentPage=${index}">${index}</a></li>
					</c:if>

					<c:if test="${pageBean.currentPage!=index}">
						<li ><a href="product?method=show&id=${param.id}&currentPage=${index}">${index}</a></li>
					</c:if>
				</c:forEach>

				<li class="${pageBean.currentPage==pageBean.totalPage?'disabled':''}">
					<a href="product?method=show&id=${param.id}&currentPage=${pageBean.currentPage+1}" aria-label="Next">
